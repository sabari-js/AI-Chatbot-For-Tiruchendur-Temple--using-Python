...
#########################
ai_name = 'CHENDUR ASSISTANT'.lower()
EXIT_COMMANDS = ['bye','exit','quit','shut down', 'shutdown']

ownerName = "User"
ownerDesignation = "Devotee"
ownerPhoto = "1"
WAEMEntry = None
...
def main(text):
		...
		if isContain(text, ['Temple videos','can you please shown some temple videos']):
			speak("OK" +ownerDesignation+", here the videos for you....",True, True)
			webScrapping.openWebsite('https://www.youtube.com/results?search_query=tiruchendur+festival+videos')
			return
		...
		if isContain(text, ['map', 'direction','map direction','direction for tiruchendur']):
		...
		if isContain(text, ['temple news','news']):
			...
			if isContain(text, ["no","don't"]):
				...
				webScrapping.openWebsite('https://www.maalaimalar.com/Topic/Tiruchendur-Temple')
				speak("You can now read the full news from this website.")
			return
		if isContain(text, ['Accommodation','room booking','residency','how to i stay']):
			speak("Ok "+ownerDesignation+", Here the Accommodation Details...", True)
			webScrapping.openWebsite('http://tiruchendur.org/accommodations.htm')
		...
		if isContain(text, ['temple history','history']):
			speak('Thiruchendur Murugan Temple is a Temple situated in Tiruchendur, Tamil Nadu dated 2000 years back.Asuran Soorabadman & thier Brothers suled veeramahendrapuri. Assuran who got wish from lord shiva had tortured devargal from heaven. As per kind request from devargal to loard shiva, he created lord murugan, who come spark from third eye. And then asuran has beeb assassinated by lord murugan and therefore tht place is known as Tiruchendur.',True)
			speak('Do you want to read Breif history ?', True)
			text = record(False, False)
			if isContain(text, ["no","don't"]):
				speak("No Problem "+ownerDesignation, True)
			else:
				speak("OK " +ownerDesignation+", here the history of temple....",True)
				webScrapping.openWebsite('https://hindutemplesblog.wordpress.com/2017/12/19/history-of-thiruchendur-murugan-temple/')	
			return

		if isContain(text, ['dutch incident','mysterious incident',"story of shunmuga statue"]):
			speak('In Thiruchendur in 1648, the Dutch smuggled the statue of Shanmugar and Natarajar by sea, believing it to be made of gold. Then the idols were thrown into the sea because of the fury of the sea. Then in the dream of Vadamalaippappillai Murugan appeared gracefully and then retrieved the idol to tell the location of the idol',True)
			speak('Do you want to read Breif details ?', True)
			text = record(False, False)
			if isContain(text, ["no","don't"]):
				speak("No Problem "+ownerDesignation, True)
			else:
				speak("OK " +ownerDesignation+", here the history of temple....",True)
				webScrapping.openWebsite('http://tiruchendur.org/dutch.htm')
			return
				
		
		if isContain(text, ['weather','weather in Tiruchendur']):
		...
		result = normalChat.chatbot_response(text)
